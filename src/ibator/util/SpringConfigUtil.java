   ...
   public static void writeSpring(File dir, List<String> springDao)
   {
     ...
     if ((beans == null) || (!beans.getName().equals("beans"))) {
       beans = document.addElement(
         QName.get("beans", "", (String)map.get("lgh")));
       Namespace namespace1 = DocumentFactory.getInstance()
         .createNamespace("", 
       ...
       beans.addAttribute(
         QName.get("schemaLocation", "xsi", "http://www.w3.org/2001/XMLSchema-instance"), 
     ...
     if (servicElement == null) {
       ...
       servicElement.addAttribute("base-package", "com.service");
     }
     ...
     {
       beans.addComment(" - - - - - - - sessionFactory - - - - - - - ");
     ...
     if (list != null)
       for (Element element : list) {
         ...
         {
           if (element.element("advisor").attributeValue("advice-ref").equals("transactionAdvice"))
             weave = element; }
       }
     if (tx == null)
     {
       beans.addComment(" - - - - - - spring 事务属性 - - - - - - - ");
     ...
     if (springDao != null)
     {
       for (String dao : springDao) {
         ...
         {
           i++;
           String daoName = dao.replaceFirst(dao.charAt(0)+"", 
             (char)(dao.charAt(0) + ' ')+"");
