...
import org.mybatis.generator.internal.util.ClassloaderUtility;
 ...
 public class DBUtil
 {
   ...
   public void closeDb() {
     try { if ((this.con != null) && (!this.con.isClosed())) {
         this.con.close();
       }
       ...
       for (TableVo.KeyInfo keyInfo : TableVo.getPkeys()) {
         ...
         if (isUnionKey(fkTableName)) {
           type = 
             ...
             "java.util.Set<" + StringUtility.getTable(fkTableName) + "Key>";
         }
   ...
   public static synchronized TableVo getKeys(String tablename)
   {
     ...
     Connection con = null;
     try {
       con =  org.apache.ibatis.ibator.internal.db.ConnectionFactory.getInstance().getConnection(jdbc);
   ...
   public static List getAllTable()
   {
	   ...
	   DBVO vo = Globar.global.getDbVo();
	  JDBCConnectionConfiguration config = new JDBCConnectionConfiguration(vo);
	  
	 
	   try {
		   con = ConnectionFactory.getInstance().getConnection(config );
   ...
   public static void setTableConfig(Connection con, org.mybatis.generator.config.TableConfiguration tc)
   {
	   String table;
       List keys;
       ResultSet rs;
       ...
       String column = getIdentity(table);//获得第一个 自增长的 列
       boolean isIdentiy = false;
       if(key.equalsIgnoreCase(column))
           isIdentiy = true;//自增长主键  判断不严谨，不过一般没有多个  自增长的 列。
       String dialect = Globar.global.getDbVo().getDialect();
       String dialectDB = null;
       String dialects[] = {
           "DB2", "MySQL", "SqlServer", "Derby", "Cloudscape", "HSQLDB", "SYBASE", "Informix"
       };
       String as[];
       int j = (as = dialects).length;
       for(int i = 0; i < j; i++)
       {
           String str = as[i];
