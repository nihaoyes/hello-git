 ...
 public class RootClassInfo
 {
   ...
   public static RootClassInfo getInstance(String className, List<String> warnings) { RootClassInfo classInfo = (RootClassInfo)rootClassInfoMap.get(className);
     if (classInfo == null) {
     ...
     for (int i = 0; i < this.propertyDescriptors.length; i++) {
       ...
       if (propertyDescriptor.getName().equals(propertyName))
       {
         ...
         if (!propertyDescriptor.getPropertyType().getName().equals(
           propertyType)) {
           this.warnings.add(
             Messages.getString("Warning.21", propertyName, this.className, propertyType));
           break;
         }
         ...
         if (propertyDescriptor.getReadMethod() == null) {
           this.warnings.add(
             Messages.getString("Warning.22", propertyName, this.className));
           break;
         }
         ...
         if (propertyDescriptor.getWriteMethod() == null) {
           this.warnings.add(
             Messages.getString("Warning.23", propertyName, this.className));
           break;
         }
