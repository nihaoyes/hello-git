   ...
   extends AbstractJavaGenerator
 {
   ...
   private InnerClass getGeneratedCriteriaInnerClass(TopLevelClass topLevelClass)
   {
     ...
     Iterator localIterator1 = this.introspectedTable.getNonBLOBColumns().iterator();
     ...
     Object strIter = criteriaLists.iterator();
     sb.append("return ");
     sb.append((String)((Iterator)strIter).next());
     sb.append(".size() > 0");
     if (!((Iterator)strIter).hasNext()) {
       sb.append(';');
     }
     method.addBodyLine(sb.toString());
     while (((Iterator)strIter).hasNext()) {
       sb.setLength(0);
     ...
     if (criteriaLists.size() < 2) {
       ...
       while (((Iterator)strIter).hasNext()) {
         method.addBodyLine(String.format("allCriteria.addAll(%s);", new Object[] { ((Iterator)strIter).next() }));
       }
     ...
     Iterator localIterator2 = this.introspectedTable.getNonBLOBColumns().iterator();
