 ...
 public class DatabaseIntrospector
 {
   ...
   public DatabaseIntrospector(TableConfiguration tc, DatabaseMetaData databaseMetaData)
   {
   ...
   private void calculatePrimaryKey(FullyQualifiedTable table, IntrospectedTable introspectedTable)
   {
       ...
       try
       {
           rs = databaseMetaData.getPrimaryKeys(null, null, table.getIntrospectedTableName());
           while(rs.next()) 
           {
   ...
   {
     Iterator localIterator = tableConfiguration.getColumnOverrides().iterator();
     while (localIterator.hasNext()) {
       ...
       if (introspectedTable.getColumn(columnOverride.getColumnName()) == null) {
         this.warnings.add(
           Messages.getString("Warning.3", columnOverride.getColumnName(), table.toString()));
       }
     }
 
     for (String string : tableConfiguration.getIgnoredColumnsInError()) {
       this.warnings.add(
         Messages.getString("Warning.4", string, table.toString()));
     }
     ...
     if ((generatedKey != null) && 
       ...
       if (generatedKey.isIdentity()) {
         this.warnings.add(
           Messages.getString("Warning.5", generatedKey.getColumn(), table.toString()));
       } else {
         this.warnings.add(
           Messages.getString("Warning.6", generatedKey.getColumn(), table.toString()));
       }
     }
   }
   ...
   public String getTableName(TableConfiguration tc2, String tableName)
   {
     ...
     FullyQualifiedTable table = new FullyQualifiedTable(
       StringUtility.stringHasValue(tc.getCatalog()) ? 
       atn.getCatalog() : null, 
       StringUtility.stringHasValue(tc.getSchema()) ? 
       atn.getSchema() : null, 
       atn.getTableName(), 
       ...
       tc.getProperty("runtimeTableName"), 
     ...
     throws SQLException
   {
     ...
     if (columns.isEmpty()) {
       this.warnings.add(
         Messages.getString("Warning.19", tc.getCatalog(), tc.getSchema(), tc.getTableName()));
       return null;
     }
     
     removeIgnoredColumns(tc, columns);//去掉忽略的类名 
     calculateExtraColumnInformation(tc, columns);//获得想对应的java field及javaType。 javaBean字段全部小写
     applyColumnOverrides(tc, columns);// 找到 有没有覆盖  列的。 覆盖 java field等属性
     calculateIdentityColumns(tc, columns);// 获得 主键的列
     ...
     while (iter.hasNext()) {
       ...
       else
       {
         reportIntrospectionWarnings(introspectedTable, tc, 
           introspectedTable.getFullyQualifiedTable()); //错误报告
       }
     }
   ...
   {
     Iterator localIterator = columns.entrySet().iterator();
   
     for (Iterator<IntrospectedColumn> tableColumns; localIterator.hasNext(); )
     {
       Map.Entry<ActualTableName, List<IntrospectedColumn>> entry = (Map.Entry)localIterator.next();
    	       ...
    	       if (tc.isColumnIgnored(introspectedColumn.getActualColumnName())) {
    	         ...
    	         if (this.logger.isDebugEnabled()) {
    	           this.logger.debug(
    	             ...
    	             Messages.getString("Tracing.3", introspectedColumn.getActualColumnName(), ((ActualTableName)entry.getKey()).toString()));
    	         }
    	       }
       }
   ...
   private void calculateExtraColumnInformation(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns)
   {
     ...
     if (tc.getColumnRenamingRule() != null) {//列别名？
     ...
     Iterator localIterator1 = columns.entrySet().iterator();
     Iterator localIterator2;
     for (; localIterator1.hasNext();)
	     {
	       Map.Entry<ActualTableName, List<IntrospectedColumn>> entry = (Map.Entry)localIterator1.next();
	       ...
	       while(localIterator2.hasNext()){
		       ...
		       boolean f = JavaReservedWords.containsWord(calculatedColumnName);//java 关键字查找
		       f = (f) || (JavaReservedWords.containsWord(JavaBeansUtil.getCamelCaseString(calculatedColumnName, false)));
		       ...
		       FullyQualifiedJavaType fullyQualifiedJavaType = this.javaTypeResolver
		         .calculateJavaType(introspectedColumn);//根据map 获得jdbc 和FullyQualifiedJavaType类型转换 
		       
		       if (fullyQualifiedJavaType != null)
		       {
		         introspectedColumn.setFullyQualifiedJavaType(fullyQualifiedJavaType);
		         introspectedColumn.setJdbcTypeName(
		           this.javaTypeResolver.calculateJdbcTypeName(introspectedColumn));
		       }
		       else {
		         boolean warn = true;
		         if (tc.isColumnIgnored(
		           introspectedColumn.getActualColumnName())) {
		           warn = false;
		         }
		         
		         ColumnOverride co = tc.getColumnOverride(
		           introspectedColumn.getActualColumnName());
		         if ((co != null) && 
   ...
   private void calculateIdentityColumns(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns)
   {
     ...
     for (Iterator localIterator1 = columns.entrySet().iterator(); localIterator1.hasNext();)
     {
       Map.Entry<ActualTableName, List<IntrospectedColumn>> entry = (Map.Entry)localIterator1.next();
       Iterator localIterator2 = ((List)entry.getValue()).iterator();
       while(localIterator2.hasNext()){
    	   IntrospectedColumn introspectedColumn = (IntrospectedColumn)localIterator2.next();
           if (introspectedColumn.getActualColumnName().equalsIgnoreCase(DBUtil.getIdentity(((ActualTableName)entry.getKey()).getTableName()))) {
           ...
           if (columnOverride != null) {
             if (this.logger.isDebugEnabled()) {
               this.logger.debug(
                 ...
                 Messages.getString("Tracing.4", introspectedColumn.getActualColumnName(), ((ActualTableName)entry.getKey()).toString()));
             }
             
             if (StringUtility.stringHasValue(columnOverride
               .getJavaProperty())) {
               introspectedColumn.setJavaProperty(
                 columnOverride.getJavaProperty());
             }
             
             if (StringUtility.stringHasValue(columnOverride
               .getJavaType()))
             {
               introspectedColumn.setFullyQualifiedJavaType(new FullyQualifiedJavaType(
             ...
             if (StringUtility.stringHasValue(columnOverride
               .getTypeHandler())) {
               introspectedColumn.setTypeHandler(
                 columnOverride.getTypeHandler());
             }
             ...
             introspectedColumn.setProperties(
               columnOverride.getProperties());
     ...
     throws SQLException
   {
     ...
     String localCatalog; String localSchema; 
      if (delimitIdentifiers) {
       ...
       if (this.databaseMetaData.storesLowerCaseIdentifiers()) {
          localCatalog = tc.getCatalog() == null ? null : 
           tc.getCatalog().toLowerCase();
          localSchema = tc.getSchema() == null ? null : 
           tc.getSchema().toLowerCase();
         localTableName = tc.getTableName() == null ? null : 
           tc.getTableName().toLowerCase(); 
         } else { 
		         if (this.databaseMetaData.storesUpperCaseIdentifiers()) {
		            localCatalog = tc.getCatalog() == null ? null : 
		             tc.getCatalog().toUpperCase();
		            localSchema = tc.getSchema() == null ? null : 
		             tc.getSchema().toUpperCase();
		           localTableName = tc.getTableName() == null ? null : 
		             tc.getTableName().toUpperCase();
		         } else {
		           localCatalog = tc.getCatalog();
		           localSchema = tc.getSchema();
		           localTableName = tc.getTableName();
     ...
     ResultSet rs = this.databaseMetaData.getColumns(localCatalog, localSchema, 
       localTableName, null);
	     ...
	     while (rs.next()) {//获取表里所有列，IntrospectedColumn
	    ...
	    //   introspectedColumn.setSequenceColumn(rs.getString("COLUMN_NAME"););
       ...
       (!StringUtility.stringContainsSQLWildcard(localTableName)))
     {
       ...
       this.warnings.add(
         Messages.getString("Warning.25", inputAtn.toString(), sb.toString()));
     }
   ...
   private List<IntrospectedTable> calculateIntrospectedTables(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns)
   {
     ...
     while (localIterator1.hasNext()) {
       ...
       }//建立表IntrospectedTable ，把 列IntrospectedColumn放入
       
       calculatePrimaryKey(table, introspectedTable);//获取主键 放入IntrospectedTable中
