 ...
 public class GeneratedKey
 {
   ...
   public GeneratedKey(String column, String configuredSqlStatement, boolean isIdentity, String type)
   {
     ...
     if (dialect == null) {
       this.runtimeSqlStatement = configuredSqlStatement; //自动返回最后一个INSERT或 UPDATE 查询中 AUTO_INCREMENT列设置的第一个表发生的值
     } else {
  ...
  //  boolean rc;
   ...
   public void validate(List<String> errors, String tableName) {
     if (!StringUtility.stringHasValue(this.runtimeSqlStatement)) {
       errors.add(
         Messages.getString("ValidationError.7", tableName));
     }
     
     if ((StringUtility.stringHasValue(this.type)) && 
       (!"pre".equals(this.type)) && (!"post".equals(this.type))) {
       errors.add(
         Messages.getString("ValidationError.15", tableName));
     }
     
 
     if (("pre".equals(this.type)) && (this.isIdentity)) {
       errors.add(
         Messages.getString("ValidationError.23", tableName));
     }
     
     if (("post".equals(this.type)) && (!this.isIdentity)) {
       errors.add(
         Messages.getString("ValidationError.24", tableName));
     }
   }
